    ...
    <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-medium text-zinc-900 dark:text-white">{{ $plan['name'] }}</div>
                                        <div class="text-sm text-zinc-500 dark:text-zinc-400">{{ $plan['description'] }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="text-lg font-bold text-zinc-900 dark:text-white">
                                    R$ {{ number_format($pricing['monthly'][$plan['id']], 2, ',', '.') }}
                                </div>
                                <div class="text-sm text-zinc-500 dark:text-zinc-400">por mês</div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="text-lg font-bold text-zinc-900 dark:text-white">
                                    R$ {{ number_format($pricing['quarterly'][$plan['id']], 2, ',', '.') }}
                                </div>
                                <div class="text-sm text-zinc-500 dark:text-zinc-400">3 meses</div>
                                <div class="text-xs text-green-600 dark:text-green-400 font-medium">
                                    Economia: R$ {{ number_format(($pricing['monthly'][$plan['id']] * 3) - $pricing['quarterly'][$plan['id']], 2, ',', '.') }}
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="text-lg font-bold text-zinc-900 dark:text-white">
                                    R$ {{ number_format($pricing['annual'][$plan['id']], 2, ',', '.') }}
                                </div>
                                <div class="text-sm text-zinc-500 dark:text-zinc-400">12 meses</div>
                                <div class="text-xs text-green-600 dark:text-green-400 font-medium">
                                    Economia: R$ {{ number_format(($pricing['monthly'][$plan['id']] * 12) - $pricing['annual'][$plan['id']], 2, ',', '.') }}
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="text-sm font-medium text-zinc-900 dark:text-white">
                                    Até {{ number_format((($pricing['monthly'][$plan['id']] * 12) - $pricing['annual'][$plan['id']]) / ($pricing['monthly'][$plan['id']] * 12) * 100, 0) }}%
                                </div>
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($pricing['promotions'] as $promotion)
                <div class="bg-gradient-to-r from-primary/10 to-primary/5 dark:from-primary/20 dark:to-primary/10 rounded-xl p-6 border border-primary/20">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h3 class="text-xl font-bold text-zinc-900 dark:text-white">{{ $promotion['title'] }}</h3>
                            <p class="text-zinc-600 dark:text-zinc-400 mt-2">{{ $promotion['description'] }}</p>
                        </div>
                        <div class="bg-primary text-white px-3 py-1 rounded-full text-sm font-semibold">
                            -R$ {{ number_format($promotion['discount'], 2, ',', '.') }}
                        </div>
                    </div>
                    
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-zinc-500 dark:text-zinc-400">
                            Válido até: {{ $promotion['valid_until'] }}
                        </div>
